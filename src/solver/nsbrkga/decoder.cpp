...
Decoder::Decoder(const Instance & instance,
    ...
    : instance(instance),
      ...
      values(num_threads,
            std::vector<double>(instance.num_knapsacks, 0.0)),
      weights(num_threads,
            std::vector<double>(instance.num_knapsacks, 0.0)),
      qs(num_threads,
...
#   ifdef _OPENMP
        ...
        std::vector<std::pair<double, unsigned>> & q = this->qs[omp_get_thread_num()];
#   else
...
#   endif
    ...
    value = std::vector<double>(this->instance.num_knapsacks, 0.0);
    weight = std::vector<double>(this->instance.num_knapsacks, 0.0);
